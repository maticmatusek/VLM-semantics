...
####################################################################################################
# 4-bit NF4 quantization config (BitsAndBytes) - optional
...
# ---------- helper: ask EuroVLM one question about one local image ----------

def ask_eurovlm(image_path: str, question: str) -> str:
    # load local image
    image = Image.open(image_path).convert("RGB")

    system_prompt = (
        "You are EuroVLM --- a multimodal AI assistant specialized in European "
        "languages that provides safe, educational and helpful answers about images and text."
    )
    ...
    # Build chat prompt and encode with image
...
# ---------- main: load JSON, run EuroVLM, save new JSON ----------
...
for idx, item in enumerate(data):
    # use local image path from "image_path"; also support "image" fallback
    raw_image = item.get("image_path", item.get("image", None))
    if raw_image is None:
        continue

    # handle both "image": "SLO_images\\00420.jpg" and "image": ["SLO_images\\00420.jpg"]
    if isinstance(raw_image, list):
        raw_image = raw_image[0]

    # normalize slashes and prepend data/
    rel_path = raw_image.replace("\\", "/")
    ...
    for key, value in list(item.items()):
        if (
            ...
            and not key.startswith("Answer")
            and "suitable" not in key
        ):
            q_key = key
            a_key = "Answer" + q_key  # e.g. F1 -> AnswerF1, SLO -> AnswerSLO

            print(f"  - Asking: {q_key} ({value[:60]}...)")
            answer = ask_eurovlm(image_path, value)
            print(f"    ANSWER: {answer}")

            # overwrite or create Answer* field with EuroVLM's answer
            item[a_key] = answer
...
print(f"Done. Saved EuroVLM answers to: {output_json_path}")
